 ...
 *
 * ── ГДЕ ЖИВОЙ ФАЙЛ ЧИСЛИТСЯ ВХОДОМ ──────────────────────────────────────────
 *
 * Эталон лежит ВНЕ проекта, поэтому он назван входом задачи `test` поимённо
 * (`nx.namedInputs.default` в манифесте зоны, `tasker:BASER2-78`). Без этого nx
 * отдавал бы зелёное из кэша, снятого до правки эталона, — то есть проба
 * доказывала бы не то, что написано в её имени.
 */
