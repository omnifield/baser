 ...
 * строка данных была одна среди четырёх, теперь она одна вообще.
 *
...
describe('РАСХОЖДЕНИЕ, названное до user: дефолт выпуска ушёл вперёд живого репо', () => {
  it('без заполнения версия 24, живой репозиторий на 22 — и это ЕДИНСТВЕННОЕ различие с ним', async () => {
    ...
    // Единственное расхождение — версия образа, и «единственное» здесь
    // буквально: не одна строка данных среди прощённых комментариев, а одна
    // строка на весь файл.
    const diff = differingLines(landed, liveArtifact(LIVE));
    expect(diff.map((item) => item.line)).toEqual([8]);
    expect(diff[0].ours).toContain('typescript-node:24');
    expect(diff[0].live).toContain('typescript-node:22');
  });
