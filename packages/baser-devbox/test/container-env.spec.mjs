 ...
 * считаются от `secretsVolume`; переопределить их своим значением значит получить
 * девбокс, который ищет креды не там, где они лежат. Поэтому здесь названный
 * отказ, а не тихая победа последнего.
 *
...
describe('продуктовые переменные выражаются настройкой', () => {
  ...
  it('рядом с секрет-моделью: считанное обвесом впереди, продуктовое следом', async () => {
    ...
    expect(text).toContain(
      'Переменные окружения контейнера: секрет-модель, продуктовые переменные.',
    );
  });
