...
describe('ЦЕПОЧКА ОБЪЯВЛЕНА ШАГАМИ, а не склеена `&&`', () => {
  ...
  it('onCreate — ТОЖЕ ЦЕПОЧКА, и его шаги названы так же', async () => {
    ...
    // Первым идёт фиксация редакции участка (`tasker:BASER2-290`): она ни от чего
    // не зависит, а отказ дальше по цепочке разбирают по вопросу «на каком образе».
    ...
    // Инструментов нет — шага нет, ровно как в постсоздании. Участок при этом на
    // месте: его редакция от перечня инструментов не зависит.
    consumer.cleanup();
    consumer = null;
