...
describe('ПЕРЕЕЗД ТОМОВ при смене пользователя — что дверь про него говорит', () => {
  it('план называет расхождение и новые пути, но про переезд НЕ говорит', async () => {
    ...
    // ── НАХОДКА, ссохшаяся до одного пункта ────────────────────────────────
    //
    // Прежний конец у двери теперь ЕСТЬ: с `tasker:BASER2-38` план говорит
    // `imageUser "node" → "vscode"`, прочитав прежнее из разложенного артефакта.
    // Половина находки — «дверь молчит о прежнем значении» — снята вместе с
    // состоянием, которое описывала (`naming.spec.mjs` утверждает это прямо).
    //
    // Осталась вторая половина, и она про СЛЕДСТВИЕ, а не про значение: дверь
    // называет движение НАСТРОЙКИ, но не то, что от неё считается. Что тома
    // `omnifield-secrets` и `omnifield-pnpm-store` поедут с `/home/node/…` на
    // `/home/vscode/…` — то есть всё, что человек клал по старому адресу,
    // окажется не там, где ищут переменные, — план не говорит. Вывести это он
    // предоставляет человеку.
    //
    // Проба стоит на оставшемся конце находки намеренно: научится дверь
    // называть и производные пути — покраснеет, и это правильный сигнал.
