...
describe('ПЕРЕИМЕНОВАНИЕ при обновлении обвеса — что дверь про него говорит', () => {
  it('разложенное поедет: план называет расхождение и ОБА конца движения', async () => {
    ...
    // ── ОБА КОНЦА ДВИЖЕНИЯ, а не только новый ──────────────────────────────
    //
    // Ради этого человек план и читает. Имя контейнера — подпись, а вот
    // `networkAlias` это ОБЕЩАНИЕ СОСЕДЯМ: сосед ходит на `http://weber-live`,
    // и после `apply` адрес станет другим. План, называющий только новое
    // значение, оставляет человека без прежнего конца — то есть без того
    // единственного, по чему он узнает своих соседей.
    //
    // Здесь стояла находка — «дверь про переименование не говорит», — и она
    // снята вместе с состоянием, которое описывала: дверь читает прежнее из
    // РАЗЛОЖЕННОГО артефакта и доказывает его побайтово (`tasker:BASER2-38`).
    //
    // Проверяется не текст целиком, а утверждение: прежнее значение стоит в
    // одной строке с новым. Так проба переживёт правку разделителя или порядка
    // колонок и покраснеет ровно тогда, когда пропадёт один из концов.
    const together = (previous) =>
      text.split('\n').filter((line) => line.includes(previous));

    const renamed = together('weber-live-devbox');
    expect(renamed, 'прежнее имя пропало из плана').toHaveLength(1);
    expect(renamed[0]).toContain('weber-devbox');

    const realiased = together('"weber-live"');
    expect(realiased, 'прежний алиас пропал из плана').toHaveLength(1);
    expect(realiased[0]).toContain('"weber"');
