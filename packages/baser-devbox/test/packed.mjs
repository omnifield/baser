...
/**
 * НАСТРОЙКИ, НА КОТОРЫХ ЖИВЁТ ЭТОТ РЕПОЗИТОРИЙ, — прочитанные, а не переписанные.
 *
 * Эталон приёмки есть функция ДВУХ вещей: обвеса и вот этих значений. Обвес
 * берётся настоящим — тарболом; значения до `tasker:BASER2-182` переписывались в
 * код пробы руками (`presets: ['omnifield']`, `imageTag: '22'`). Пока набор не
 * менялся, снимок совпадал с оригиналом — а стоило корню получить ЛЮБУЮ новую
 * настройку, и «артефакт совпадает с живым» краснело на исправном обвесе.
 * Ровно так и вышло, когда репозиторий заполнил `npmScope`.
 *
 * Класс дефекта у зоны уже назван на пробах гейта выпуска (`tasker:BASER2-176`):
 * там был зашит номер, здесь — набор настроек, а посылка одна. **Проба обязана
 * судить ФАКТ** («обвес на настройках ЭТОГО репозитория даёт ровно его
 * `.devcontainer`»), а не снимок, снятый в день её написания. Читать живой файл
 * приёмке не в новость: `liveArtifact()` читает живой артефакт — здесь тот же
 * приём, только со стороны входа. Второй копии набора не существует.
 *
 * Читает файл ДВЕРЬ, своим публичным входом, а не разборщик этой фикстуры:
 * зависимостей у обвеса нет вовсе (YAML читать нечем), а свой разбор был бы
 * ВТОРОЙ правдой о файле — и разъехался бы с дверью там, где мы не подумали.
 * Заодно приезжает и проверка формы: непригодный файл называет причину, а не
 * молча подсовывает приёмке дефолты.
 */
export function liveTuning() {
  const declaration = readSourceDeclaration(packedManifest());
  if (!declaration.ok) {
    throw new Error(
      `объявление обвеса непригодно, читать настройки нечем:\n${describeProblems(declaration.problems)}`,
    );
  }

  const log = new DoorProblemLog();
  const state = readSourceConfig(readRepo(REPO_ROOT), declaration.value, log);
  // Файла нет — для двери это законное «ничего не выбрано», а для приёмки
  // сорванная посылка: сверять живой артефакт стало бы не с чем, и получилось
  // бы утверждение про дефолты выпуска под именем «настройки живого репозитория».
  if (!state.existed) {
    throw new Error(
      `у этого репозитория нет файла настроек (${state.path}) — ` +
        'приёмке нечем воспроизвести его собственный .devcontainer',
    );
  }
  if (state.config === null) {
    throw new Error(
      `настройки этого репозитория (${state.path}) непригодны:\n${describeProblems(log.list())}`,
    );
  }

  return tuning(state.config);
}
