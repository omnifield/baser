 ...
 *
 * Рядом с правилами про `dest` живут и АДРЕСА, В КОТОРЫЕ ЦЕЛИТЬСЯ НЕЛЬЗЯ: они
 * часть тех же правил.
 */

/**
 * Перечень поставленного у потребителя — `baser.json` в корне репозитория.
 *
 * Имя лежит здесь, а не на стороне потребителя, потому что читателей у него два
 * и они на разных половинах формы: `config.ts` этот файл РАЗБИРАЕТ, а
 * `declaration.ts` требует в него НЕ ЦЕЛИТЬСЯ
 * (`artifact-over-consumer-config`). Одна константа на обе половины — второе
 * место для одного имени могло бы разъехаться.
 */
