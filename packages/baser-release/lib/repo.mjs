 ...
 * `publicNames`): факт репозитория не перестаёт им быть оттого, что спрашивает
 * его не гейт, а конвейер выпуска.
 *
...
/**
 * КАРТА ИМЁН РЕПОЗИТОРИЯ, СВЕРЕННАЯ С ЕГО РАСКЛАДКОЙ, — то, что зона отдаёт
 * наружу: `{внутреннее имя → публичное}` по всем пакетам сразу.
 *
 * Целиком, а не по одному имени, потому что вместе с именем пакета конвейер
 * переписывает имена его межпакетных зависимостей: спросив про один пакет, он
 * не узнал бы, как теперь зовётся сосед в его `dependencies`.
 *
 * СВЕРКА ЖИВЁТ ЗДЕСЬ, а не только в пробе. Карта, в которой пакета нет, — это
 * карта, по которой нельзя выпускать: отдай её половиной, и конвейер доедет до
 * `npm publish` с внутренним именем. Лишняя запись отвергается с другого конца:
 * она говорит о пакете, которого нет, — либо его сняли и забыли карту, либо в
 * имени опечатка, и тогда настоящий пакет не объявлен вовсе.
 *
 * Прежние имена сверяются заодно: они часть той же карты и разъезжаются с
 * раскладкой так же молча.
 *
 * @param {string} root корень судимого репозитория
 * @returns {Map<string, string>} внутреннее имя → публичное, по порядку имён
 */
export function publicNames(root) {
  const packages = readPackages(root);
  const declared = readPublicNames(root);
  const formerNames = readFormerNames(root);
  const known = new Set(packages.map((pkg) => pkg.name));

  const безымянные = packages
    .filter((pkg) => !declared.has(pkg.name))
    .map((pkg) => pkg.name);
  ...
  const ничьи = [
    ...[...declared.keys()].map((name) => ({ file: PUBLIC_NAMES, name })),
  ...
  return new Map(
    packages
      .map((pkg) => pkg.name)
      .sort()
      .map((name) => [name, /** @type {string} */ (declared.get(name))]),
  );
}
