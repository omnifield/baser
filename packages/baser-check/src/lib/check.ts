...
export function checkPackage(
  ...
  if (renderable.length === 0) {
    // Два разных состояния носят одно машинное имя `skipped`, и различает их
    // ТОЛЬКО эта строка (`tasker:BASER2-130`). Выше слой пропускается потому,
    // что сверять физически нечем — манифест не прочитан, объявление
    // непригодно; там прогон и без того красный, и «пропущено» читается верно.
    // Здесь же прогон ЗЕЛЁНЫЙ, и «пропущено» рядом с четырьмя `ok` заставляет
    // человека искать несделанную работу — хотя её нет: обвес без единой
    // рендеримой записи получил полный ответ. Поэтому причина названа итогом
    // («0 из N», непроверенного не осталось), а не отсутствием возможности.
    skip(
      'templates',
      `мерить нечего: рендеримых записей в раскладке 0 из ${declaration.layout.length} — ` +
