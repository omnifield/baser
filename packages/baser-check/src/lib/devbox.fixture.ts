...
/**
 * Адреса настроек живого обвеса, у которых дефолт ВЫЧИСЛЯЕМЫЙ (`defaultFrom`).
 *
 * Одним местом на всю зону: два теста называют этот список целиком, и разъехаться
 * им нельзя. Порядок — тот, в котором настройки отдаёт разбор контрактов
 * (по байтам ключа), а не тот, в котором они лежат в манифесте.
 *
 * Список НАЗЫВАЕТ имена, а не берёт их из объявления: имя настройки — часть
 * адреса, по которому потребитель чинит свой обвес, и подстановка его из того же
 * объявления превратила бы пробу в тавтологию. Переименование настройки обязано
 * красить пробу — так и случилось, когда `runtimeVersion` стал `imageTag`
 * (`tasker:BASER2-83`).
 */
export const RESOLVER_SETTINGS = [
  'package.json.baser.settings.imageTag.defaultFrom',
