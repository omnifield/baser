 ...
 * резолвер отдавал 22, сегодня 24») дверь назвать не может: прежнего ЗНАЧЕНИЯ
 * никто не хранит. Паспорт укладки при этом существует и лежит в
 * `baser.lock.json` (`tasker:BASER2-4`) — но помнит он, ЧТО положено: `dest`,
 * `src`, обвес, версию обвеса, класс, хеш. С какими значениями рендерилось —
 * не помнит, и место под это не заведено.
 *
 * Разница не словесная: «памяти нет вовсе» отправляло бы заводить её с нуля,
 * а на самом деле есть куда класть — не хватает решения, что именно класть, и
 * это разговор про форму записи, то есть шов между зонами
 * (`tasker:BASER2-38`). Дверь называет то, что знает достоверно, и не выдумывает
 * второй конец. Расхождение самого артефакта при этом названо всегда — его
 * несёт `step.previous` движка.
 */
