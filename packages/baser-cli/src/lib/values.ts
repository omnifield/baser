 ...
 *
 * ЧЕГО ЗДЕСЬ НЕТ И ПОЧЕМУ. Движение дефолта МЕЖДУ ВЫПУСКАМИ обвеса («вчера
 * резолвер отдавал 22, сегодня 24») дверь назвать не может: прежнее значение
 ...
 * самого артефакта при этом названо всегда — его несёт `step.previous` движка.
 */
...
export async function loadDefaults(
  ...
  const context: ResolverContext = {
    ...
    source: {
      ...
      // Обвес версию не назвал — резолвер получает ПУСТУЮ строку, а не `0.0.0`.
      //
      // Форма контракта требует здесь `string` (`baser-contracts`, зона не
      // наша), поэтому назвать отсутствие тем же `null`, что в паспорте
      // укладки, дверь сегодня не может. Из двух строк, которыми можно
      // изобразить неизвестное, выбрана та, что видна: пустая подставится в
      // шаблон пустым местом, а `0.0.0` выглядела бы настоящей версией и дала
      // бы правдоподобно неверный результат — уверенный указатель не туда хуже
      // отсутствующего.
      //
      // Правильная форма — `string | null`, как у паспорта; это правка
      // контракта, и она вынесена наверх предложением (`tasker:BASER2-68`).
      version: pkg.version ?? '',
    },
  };
