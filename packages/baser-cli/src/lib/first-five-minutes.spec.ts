...
describe('BASER2-34 · Node на хосте — предусловие, а не данность', () => {
  it('без Node на PATH прямой запуск не проходит', () => {
    const into = built();
    ...
    // ничего, кроме доки.
    //
    // Название пробы говорило «предусловие настоящее», и это было утверждение
    // СИЛЬНЕЕ того, что она доказывает: пустота `/usr/bin:/bin` тут не строится,
    // а угадывается — в этом образе Node лежит в `/usr/local/bin`, а в Alpine
    // ровно в `/usr/bin`, и там проба покраснеет, проверив занятое. Чинится
    // подставлением заведомо пустого каталога (`tasker:BASER2-56`); до тех пор
    // проба утверждает то, что видит, — README помечен той же сноской.
    expect(() =>
