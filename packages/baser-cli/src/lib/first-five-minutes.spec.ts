...
describe('BASER2-34 · Node на хосте — предусловие, а не данность', () => {
  it('без Node на PATH прямой запуск невозможен — предусловие настоящее', () => {
    const into = built();
  ...
  it('оговорка про имя репозитория названа и помечена временной (BASER2-32)', () => {
    const doc = readFileSync(join(built(), 'INSTALL.md'), 'utf-8');

    // Пока имя девбокса приезжает от точки монтирования, дока обязана про это
    // сказать. И обязана сказать, что это временно, — костыль, переживший свою
    // причину, становится ложью в доке.
    expect(doc).toContain('BASER2-32');
  });
