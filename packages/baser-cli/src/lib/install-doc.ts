...
**Обвес в твой репозиторий не кладётся вообще.** Он лежит в этой папке, и дверь
...
поставку достаёт дверь, и названный каталог для неё такой же законный источник,
как склад.
...
Регулировки этого обвеса живут в \`${tuning}\` — твоём файле настроек. Дверь
...
файл дверь не трогает и не удаляет ни при каких условиях.
...
покажет двери каталог поставки в этой папке.

Рядом лежит \`baser.mjs\` — та же дверь, вызываемая напрямую. Каталог поставки
...
Без \`--source\` дверь пойдёт за поставкой на склад — и не найдёт её там, пока
обвес не выпущен. Отказ при этом будет названный: адрес склада, чего не хватает
и что сделать.

Когда обвес выйдет на склад, эта папка станет не нужна вовсе: дверь достанет
