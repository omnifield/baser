 ...
 * всё сразу.
 *
 * Раскомментируются только строки вида `<объявленная настройка>: …` и их
 * продолжения-списки; пояснения остаются пояснениями. Именно так проверяется,
 * что файл рабочий: `# ` стоит ПОСЛЕ структурного отступа, и снятие двух
 * символов обязано оставить валидный YAML на своём уровне вложенности.
 */
...
describe('рождение: один раз, если файла нет', () => {
  ...
  it('РАСКОММЕНТИРОВАННЫЙ дефолт разбирается и даёт то же значение', async () => {
    // Родившийся файл обязан быть рабочим, а не декоративным: строка пишется
    ...
    box.write(
      TUNING,
      uncommentSettings(box.read(TUNING) as string, [...named.keys()]),
    );
