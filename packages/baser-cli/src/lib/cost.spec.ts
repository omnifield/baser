 ...
 *
 * **`tasker:BASER2-180`** — четвёртая, и она про ту же цену с другого конца:
 * дверь называет пересоздание среды и молчит о том, что пересборка меняет саму
 * МОДЕЛЬ рабочего каталога. Незапушенное остаётся в прежнем каталоге, а человек
 * видит чистый клон и считает работу потерянной (живой переезд 2026-08-04).
 *
 * ── ЧТО ЗДЕСЬ СУДИТСЯ ───────────────────────────────────────────────────────
 *
...
describe('ЧТО ОСТАЁТСЯ ЧЕЛОВЕКУ: дверь переписывает файл, а не запущенное', () => {
  it('план говорит, что применением работа НЕ КОНЧИТСЯ', async () => {
    ...
    // пересоздать, соседей поправить, а докер консоль не зовёт и не начнёт.
    expect(text).toContain('пока ты сам его не пересоздашь');
    expect(text).toContain('обещание соседям');
    expect(text).toContain('докер консоль не зовёт и контейнерами не управляет');
  ...
  it('ПЕРЕД ПЕРЕСБОРКОЙ: применение говорит про коммит и ПУШ', async () => {
    // `tasker:BASER2-180`, живой переезд 2026-08-04. Процедура ведёт человека к
    // пересборке, а пересборка меняет модель рабочего каталога: среда,
    // поднимающаяся клонированием в том, встречает чистым клоном, и
    // незапушенное остаётся в прежнем каталоге. Человек считает работу
    // потерянной, хотя она цела, — и переделывает сделанное.
    //
    // Руководство это правило несёт (`kb:BASER3-14`), но читают его один раз, а
    // «пересобрать» нажимают под конец долгой работы. Значит сказать обязана
    // дверь — в тот момент, когда человек к пересборке подходит.
    const box = await movedUser();

    const text = renderText(await run({ command: 'apply', ...box.door }));

    expect(text).toContain('ПЕРЕД пересборкой');
    expect(text).toContain('ЗАПУШИТЬ');
    // Названа ПРИЧИНА, а не одно указание: без неё правило читается как ритуал,
    // а ритуал под конец долгой работы пропускают.
    expect(text).toContain('рабочая папка у неё бывает НЕ ТА ЖЕ САМАЯ');
    // И проверка — по ВСЕМ веткам: на живом переезде забытая ветка с работой
    // прошла бы проверку текущей ветки и пропала молча.
    expect(text).toContain('git log --branches --not --remotes');
    // Стоит внутри блока «остаётся человеку», а не отдельной строкой где-то
    // выше: это одна из работ, которую дверь за человека не делает.
    expect(text.indexOf('остаётся человеку')).toBeLessThan(
      text.indexOf('ПЕРЕД пересборкой'),
    );
  });

  it('предупреждение АДРЕСНОЕ: первой укладке пересобирать нечего', async () => {
    // Узость обещания — та же, что у всего блока. Приклеенное к каждому
    // применению, предупреждение обесценило бы себя ровно там, где оно должно
    // сработать.
    consumer = installDevbox({
      config: CONFIG,
      tuning: { [DEVBOX_ID]: OMNIFIELD },
    ...
    const text = renderText(await run({ command: 'apply', ...consumer.door }));

    expect(text).not.toContain('ПЕРЕД пересборкой');
    expect(text).not.toContain('ЗАПУШИТЬ');
  });

  it('движений нет — предупреждение остаётся: пересборка предстоит', async () => {
    // Сжатая форма блока (`tasker:BASER2-135`) режет то, что про ЗНАЧЕНИЯ.
    // Пересборка от значений не зависит вовсе: файл переписан, значит среду
    // пересоздавать, значит и коммит с пушем — до неё.
  ...
  it('план говорит то же самое: узнать надо ДО применения', async () => {
    // `plan` отвечает не про себя, а про то, что случится при применении, —
    // и последовательность у человека одна: план, применение, пересборка.
    const box = await movedUser();
