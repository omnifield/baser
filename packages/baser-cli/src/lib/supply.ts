...
export interface Supply {
   ...
   * Это ровно то, что спрашивает приёмка: повторный прогон обязан брать из кэша.
   */
...
class Store {
  ...
  /** Последняя доступная версия — спрашивается у склада, а не угадывается. */
...
interface NpmFailure {
  readonly ok: false;
  readonly reason: 'missing-manager' | 'failed';
