        ...
        `${JSON.stringify(
          {
            ...
            version: '0.0.0',
            description:
              'кэш поставок baser: каталог создан консолью, правят его не руками',
          },
          null,
          2,
        )}\n`,
  ...
  #refuseFetch(
    ...
    if (failed.reason === 'missing-manager') {
      return refuse(
        ...
        'npm',
        `поставку "${what}" достаёт консоль, и достаёт она тем же пакетным менеджером — ` +
          'а `npm` в этой среде не нашёлся. Консоль ставится в среду один раз, вместе ' +
      ...
      return refuse(
        ...
        version === null
          ? `пакетный менеджер на этой машине настроен НЕ ХОДИТЬ на склад (offline), ` +
            ...
            'день, и из кэша консоль на него не отвечает. Выходов два: закрепить версию ' +
