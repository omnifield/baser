 ...
 *
 * **Не спрашивает про `CHMOD`, и это названное ограничение** (`tasker:BASER2-223`).
 * Приведение режима записью не является: POSIX разрешает его владельцу файла
 * (и суперпользователю), а не тому, у кого есть бит записи, — и наоборот, файл
 * `444`, лежащий под нами, записи не даст, а `chmod` на нём пройдёт. Спросив
 * `access(W_OK)`, дверь отказала бы в работе, которая прошла бы, — а это хуже
 * молчания: отказ останавливает весь прогон. Своего же ответа на вопрос «смогу
 * ли я сменить режим» у ядра нет — `access` про режим не спрашивают. Поэтому
 * такой отказ остаётся поздним: его ловит сброс и говорит `flush-failed` с
 * путём и словом ядра (`EPERM`).
 *
...
export function unwritable(
  ...
  for (const write of writes) {
    // Приведение режима не запись — вопрос к нему другой, и `access` на него не
    // отвечает (см. шапку). Молча пропускаем ЗДЕСЬ, называя это там.
    if (write.kind === 'CHMOD') {
      continue;
    }
 ...
 */
export function describeRefusal(refusal: WriteRefusal, runner: Runner): string {
