  ...
  return config.existed
    ...
    : {
        ...
        message:
          ...
          // Откуда берётся содержимое — вопрос только к перегенерируемым: у
          // `placed-once` дверь ничего не собирает, и говорить про сборку от
          // дефолтов там значило бы пугать ценой, которой нет.
