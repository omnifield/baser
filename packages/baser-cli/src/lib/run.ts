...
function engineInput(
  ...
  return {
    ...
    layout: declaration.layout.map((item) => ({
      ...
      //
      // У движка это поле трёхзначное: не названо — порта не касаемся вовсе, и
      // файл остаётся с тем режимом, который у него был. Разбор формы третьего
      // состояния не отдаёт: `executable` там приведён к явному булеву, и
      // «не сказано» уже стало `false` (`baser-contracts`, `parseExecutable`).
      //
      // Поэтому наверх едет только `true` — единственное, про что консоль
      // знает, что обвес это СКАЗАЛ. Подставить `false` за молчащего обвеса
      // значило бы снимать бит с каждого сегодняшнего артефакта по утверждению,
      // которого никто не делал. Различить «сказано false» и «не сказано» здесь
      // нечем, и чинится это не тут: вопрос вынесен наверх комментом к задаче.
