...
describe('словарь — тот, что в каноне', () => {
  ...
  it('справка не говорит «дверь» — слово выведено из языка', () => {
    // Решение user 2026-08-06 (`kb:BASER3-29`): слово значило разом две вещи —
    // вход в машину снаружи (`:8080`) и наш командный фронт. За фронтом осталось
    // «консоль», и первой это обязана знать справка: её читает человек, впервые
    // набравший `baser`, и читает раньше любой нашей доки.
    //
    // Проба судит ВИДИМЫЙ текст, а не зону целиком: внутренние комментарии
    // выводятся из языка отдельной работой и лениво — сплошная правка 186 файлов
    // конфликтует со всем, что в работе, и читателю не даёт ничего
    // (`tasker:BASER2-201`).
    expect(USAGE).not.toMatch(/двер/i);
    expect(USAGE).toContain('консоль');
  });

  it('справка называет команду привязки поставки первой', () => {
    // Это первая поверхность пользователя (`kb:BASER3-33`): с неё начинается
    // любой сценарий, и до `tasker:BASER2-201` её не было вовсе — `baser.json` в
    // пустой локации писали руками.
    expect(USAGE).toContain('baser add <пакет>');
    expect(USAGE).toContain('объявить поставку в локации и применить');
  });
