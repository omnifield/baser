...
export function bundle(source: string, options: BundleOptions): BundleReport {
  ...
  stage(
    'docs',
    () => {
      writeFileSync(join(target, 'INSTALL.md'), installDoc(packed), 'utf-8');
      return 1;
    },
    () => 1,
  );
 ...
 * бандл, читает про то, что у него в руках, а не про абстрактный обвес.
 */
function installDoc(packed: PackReport): string {
...
Ничего доставать из сети не придётся — нужен только Node.

## Сухой прогон — посмотреть, что будет

\`\`\`
node ${relative('.', 'install.mjs')} --plan --cwd /путь/к/репозиторию
\`\`\`

Артефакты не раскладывает и ничего не перезаписывает. Трогает ровно две вещи —
то же, что тронул бы \`npm i\`:

- \`node_modules/\` цели — туда кладётся сам обвес (git его не отслеживает);
- \`package.json\` — обвес добавляется в \`dependencies\` одной строкой.

Дверь ищет обвес ровно там, где его нашёл бы пакетный менеджер, поэтому и
кладётся он так же. Дальше сухой прогон только показывает план.

## Установка

\`\`\`
node install.mjs --cwd /путь/к/репозиторию
\`\`\`
...
\`\`\`
node install.mjs --cwd /путь/к/репозиторию --confirm .devcontainer/devcontainer.json
\`\`\`
...
Обвес уже стоит в \`node_modules\`, и звать дверь можно напрямую:
...
Когда обвес выйдет в реестр, \`npm i ${name}\` заменит поставку на месте —
