...
export function renderCheck(report: CheckReport): string {
  ...
  lines.push(
    '',
    // Словарь канона (`kb:BASER2-9`, поправлен 2026-07-29): ДЕТАЛЬ — то, что
    // станок производит, то есть сам продукт; обвес подходит ПОСАДОЧНОМУ МЕСТУ,
    // из которых патрон лишь одно. Прежняя пара слов была расхождением внутри
    // канона, а не самодеятельностью зоны, — поэтому зона приводится следом за
    // ним, а не закрепляет одно из двух чтений молча.
    report.ok
