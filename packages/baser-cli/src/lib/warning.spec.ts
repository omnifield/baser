 ...
 * печатает заготовку.
 *
 * Обвес здесь СВОЙ, а не `@omnifield/baser-git`: настоящий git-обвес поля пока
 * не объявляет (форма 6), и объявить его — работа его зоны. Проба поэтому
 * повторяет его фигуру (хук в `.git/hooks`, `executable: true`), а не подделывает
 * его личность.
 */
 ...
 * резолвер читает то, что тот записал. Своего `git` он при этом не запускает, и
 * это не упрощение: резолвер исполняется ВНУТРИ процесса прогона, то есть
 * унаследовал бы окружение пробы целиком — ровно тот случай, из-за которого у
 * этой зоны нет своего входа к процессам (`sealed.fixture.ts`,
 * `tasker:BASER2-179`). Вложенный `git` ушёл бы по адресу из `GIT_DIR` и
 * ответил бы про НАШ репозиторий, а не про поднятый пробой.
 */
const ASKS_GIT = `
