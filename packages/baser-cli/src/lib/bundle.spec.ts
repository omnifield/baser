...
describe('INSTALL.md написан про КОНКРЕТНЫЙ обвес', () => {
  ...
  it('не говорит «дверь» — судится СОБРАННЫЙ файл, а не исходник доки', () => {
    // Слово выведено из языка (`kb:BASER3-29`, решение user 2026-08-06), а
    // `INSTALL.md` человек читает в первые пять минут знакомства с
    // инструментом — раньше README и раньше справки (`tasker:BASER2-204`).
    //
    // Проба смотрит на файл В ВЫДАЧЕ, а не на шаблон в `install-doc.ts`:
    // между «написано в исходнике» и «доехало до получателя» у этой зоны уже
    // стояла разница, и ловить её надо там, где она наступает.
    requireBuiltDoor();
    const into = join(box('bundle'), 'выдача');
    bundle(DEVBOX, { into });
    ...
    expect(doc).not.toMatch(/(?<![А-Яа-яЁё])двер/i);
    // Замена состоялась, а не текст вырезали: инструмент в доке назван.
    expect(doc).toContain('консоль');
  });
});
