...
export const USAGE = `baser-registry — магазин локации: раздача, пока жив контейнер, и склад, который её переживает
  ...
  baser-registry publish [папка]  опубликовать на склад этой локации: партию постройки
                                  либо названный пакет

  --json     ответ данными; текст — рендер поверх них
  --help     эта подсказка
  --version  версия формы ответа и версия пакета

ПУБЛИКАЦИЯ — ЭТО ТРИ ДЕЙСТВИЯ, и отказывают они порознь. Какое из них сказало
«нет», видно в ответе сразу — строкой «три действия» в тексте и полем
publication в --json, а не по формулировке отказа:

  выпуск      вещь замерзает и получает номер.
              Отказ: под этим номером на складе уже лежит ДРУГОЕ содержимое —
              либо номер занят чужим, либо вы правите выпущенное. Выпуск вещь
              замороженная: чинится поднятием номера, а не повтором команды.
              Тот же товар под тем же номером отказом НЕ является.
  отгрузка    товар попадает на склад по адресу.
              Отказ: до склада нет дороги (магазин закрыт), вход закрыт,
              назначение не то (менеджер собрался в чужой реестр).
  объявление  товар появляется на витрине.
              Витрины у магазина локации нет вовсе, и это рабочее состояние:
              склад отдаёт тому, кто знает имя. Поэтому объявление в ответе
              ПРИСУТСТВУЕТ и молчит — молчание названо, а не пропущено.

Повтор той же публикации спокоен: выпуск говорит «тот же выпуск уже был»,
отгрузка — «товар уже лежал», код возврата 0. Разошлось содержимое под тем же
номером — отказ выпуска с кодом 2, и склада он не касается.

ВИДИМОСТЬ: по умолчанию раздача отвечает СОСЕДЯМ по сети локации — магазин для
того и заводят, чтобы соседний контейнер взял у вас пакет. Сосед приходит по
имени вашей локации и порту раздачи (http://<имя локации>:4873); имя задаёт
тот, кто поднимал контейнер, а не магазин. Наружу машины при этом не выходит
ничего: хост-портов контейнер не публикует.
Запереть раздачу в петле можно настройкой host: 127.0.0.1 в config.yml
магазина. Кому она отвечает сейчас, говорит status — строкой «видимость» и
полем reach в --json.

ЧТО ОТГРУЖАЕМ И КУДА решает ПОСТРОЙКА, и решение лежит в её схеме —
.omnifield/omnifield-registry.yaml клона: нужен ли ей магазин, какая партия
уезжает на склад, по какому адресу. Файла нет — постройка ничего не объявила,
и это законное состояние: publish без аргумента положит пакет из текущего
каталога, как и раньше. Файл есть — publish без аргумента отгружает ПАРТИЮ,
а названный путь сильнее объявленного.
Решения переживают пересоздание контейнера (они в схеме), склад переживает
остановку (он на участке). Одно другое не подменяет.
