...
describe('README не расходится с тем, что делает код', () => {
  ...
  it('дока называет ТРИ ДЕЙСТВИЯ публикации и коды их отказов', () => {
    // Человек читает доку до того, как получит отказ. Не найдя здесь разницы
    // между «поднять номер» и «поднять магазин», он пойдёт чинить не то
    // (`tasker:BASER2-287`).
    for (const step of ['выпуск', 'отгрузка', 'объявление']) {
      expect(readme, `действие ${step}`).toContain(step);
    }
    for (const code of ['release-frozen', 'release-unjudged', 'publication']) {
      expect(readme, `код ${code}`).toContain(code);
    }
    // И тишина объявления названа рабочим состоянием, а не пробелом.
    expect(readme).toContain('тишина, а не отказ');
  });
