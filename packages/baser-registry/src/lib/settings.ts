...
export interface ShopSettings {
   ...
   * считается от КОРНЯ ЛОКАЦИИ, а не от каталога вызова, — иначе настройка
   * лечила бы только половину случая.
   *
...
export const DEFAULT_SETTINGS: ShopSettings = {
  ...
  // Путь от корня локации. Совпадает с раскладкой (`layout.ts`) и обязан
  // совпадать: проба это проверяет, иначе два места разъедутся молча.
  log: '.baser-registry/runtime/shop.log',
};
...
const DECLARATIONS: readonly SettingDeclaration[] = [
  ...
  },
  {
    ...
    description:
      ...
      'вы прочитаете здесь. Относительный путь считается ОТ КОРНЯ ЛОКАЦИИ, а не\n' +
 ...
 *
 * Относительный путь считается от КОРНЯ ЛОКАЦИИ, а не от каталога вызова. Это
 * не удобство: команда ставится глобально и зовётся откуда угодно, а лог,
 * посчитанный от текущего каталога, оставлял бы файл там, где человек случайно
 * стоял, — ровно тот мусор, который поймало ревью первого коммита.
 */
export function logPath(settings: ShopSettings, root: string): string {
  return resolve(root, settings.log);
}
...
export function settingsTemplate(): string {
  const head = [
    '# Магазин локации: раздача пакетов — настройки инструмента @omnifield/baser-registry.',
    '#',
    ...
    '#',
    '# Имя файла считается из личности инструмента (omnifield/registry) — по той',
    '# же конвенции, что у соседей в этой папке. Товар и состояние запуска лежат',
    '# отдельно, в .baser-registry/: они машинные, их не правят и не коммитят.',
    '',
  ];
