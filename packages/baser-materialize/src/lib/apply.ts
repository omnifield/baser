 ...
 *
 * РЕЖИМ АРТЕФАКТА ЕДЕТ ВМЕСТЕ С СОДЕРЖИМЫМ (`tasker:BASER2-214`): объявленный
 * раскладкой, он доносится до порта тем же шагом, что кладёт содержимое, и
 * только там (`declareMode`). Журнал отката режима не хранит и не переигрывает —
 * прочитать нынешний режим порт не умеет, а дерево, пережившее откат, на диск не
 * уезжает вовсе.
 */
...
/**
 * Доносит объявленный режим до порта — сразу за содержимым того же шага.
 *
 * ТРИ СОСТОЯНИЯ, А НЕ ДВА. Шаг без поля `executable` (обвес режим не назвал)
 * порта не касается вовсе: раннер оставит файлу тот режим, который у него был.
 * Названный `false` доносится наравне с `true` — это утверждение обвеса «не
 * программа», и подменять его молчанием нельзя (`tree.ts`, `setExecutable`).
 *
