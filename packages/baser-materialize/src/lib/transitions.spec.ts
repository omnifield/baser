...
describe('переход: dest существует, записи в паспорте укладки нет', () => {
...
/**
 * Зоны, где движок не видит СОБСТВЕННЫХ артефактов, быть не должно: снятая
 * запись оставляла бы вечную сироту, а план при этом рапортовал бы сходимость.
 * Тихий сирота хуже громкого конфликта.
 *
 * Раньше такую зону создавала оптимизация скана — проход по дереву пропускал
 * служебные каталоги ради скорости. Поиск по записям паспорта укладки о каталогах
 * не знает вовсе, поэтому слепых зон у него нет по построению; тесты ниже держат
 * именно это свойство, а не аккуратность обхода.
 */
