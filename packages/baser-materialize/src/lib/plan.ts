 ...
 * `reason` и `detail`.
 *
 * ЗДЕСЬ ПЕРЕЧЕНЬ РАБОТ, А НЕ ВЕСЬ ОТВЕТ ПРОГОНА (`tasker:BASER2-236`). Текст
 * говорит про то, что движок СДЕЛАЕТ с деревом: шаги, отказы владения,
 * извещения. Слово обвеса про эту локацию (`plan.warning`) сюда не печатается —
 * причина названа в конце тела: у него есть своё место, и оно занято.
 */
export function describePlan(plan: MaterializationPlan): string {
  ...
  // СЛОВА ОБВЕСА ЗДЕСЬ НЕТ, И ЭТО НЕ ЗАБЫТАЯ СТРОКА (`tasker:BASER2-236`).
  //
  // Пока места человеку не было, эта строка его заменяла: движок печатал
  // `plan.warning` последним пунктом, чтобы предупреждение не пропало вовсе.
  // Место появилось — дверь говорит слово обвеса в жанре «остаётся человеку»,
  // где названо, чего она не сделает сама, — и строка стала ВТОРЫМ печатанием
  // одного и того же в одном выводе: человек читает предупреждение дважды,
  // второй раз — среди перечня работ, где ждёт шагов, а не предостережений.
  //
  // Подавить её на стороне двери нельзя: `describePlan` зовётся целиком, и обход
  // означал бы вторую правду о том, как выглядит план. Поэтому снято здесь.
  //
  // ПОТЕРИ НЕТ: предупреждение — ДАННЫЕ плана (`plan.warning`), они уезжают в
  // машинный ответ (`--json`) и в трейс (`plan.warning`) как уезжали. Тот, кто
  // рендерит план САМ и хочет напечатать слово обвеса, читает поле — тем же
  // способом, каким его читает дверь. Печатать его текстом плана — значит
  // ставить второе место туда, где уже есть первое.
  return lines.join('\n');
}
