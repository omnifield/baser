 ...
 * и не разбирая содержимое заново. Для этого нужны пять вещей: чей это обвес,
 * какой версии формы он соответствует, что он положит потребителю и чем станок
 * будет каждый артефакт держать, из чего состоит поставка и чем она проверена.
 * Ровно они здесь и лежат.
 *
 * ## Почему это не «вторая правда»
 *
 * Правда об обвесе живёт в самой нагрузке — в его `package.json`. Опись её не
 ...
 * «не могу сказать» в `ok: true` значило бы выдать обвес, про который мы не
 * знаем, что в нём.
 */
 ...
 *
 * `2` — в артефактах появился `class` (`tasker:BASER2-63`). Поле обязательное, а
 * значит форма поехала: читатель, которому класс важен, по этому числу узнаёт,
 * есть ли класс в описи вообще, — иначе «класса нет» и «опись старая» выглядели
 * бы для него одинаково.
 */
...
/** Что обвес положит потребителю: адрес артефакта и откуда он берётся. */
export interface PayloadArtifact {
  ...
  /**
   * Чем станок держит этот артефакт: перегенерирует целиком (`regenerated`) или
   * кладёт однажды и дальше не трогает (`placed-once`).
   *
   * Едет сюда потому, что без него принимающая сторона не отличает артефакт,
   * который перепишется на следующем обновлении, от того, который человек
   * заполняет руками, — а опись существует ровно для того, чтобы узнавать груз
   * не вскрывая его. Значения объявляет ФОРМА (`ArtifactClass` контрактов), и
   * упаковка их не перечисляет у себя: новое слово формы доезжает сюда само.
   */
...
export interface PayloadManifest {
  ...
  /**
   * Версия формы объявления, которую объявил САМ ОБВЕС.
   *
   * Читайте адрес, а не имя: у соседнего звена цепи, в ответе `baser-check`,
   * поле с тем же именем значит ДРУГОЕ — версию формы, которую поддерживает
   * проверяющий, то есть этот baser. Здесь же лежит то, что обвес сказал о себе
   * сам, перенесённое из его объявления как есть. В цепочке `check → pack` эти
   * два поля лежат рядом, и потребитель, ветвящийся по коду, спутает их первым.
   */
...
export function buildPayloadManifest(input: ManifestInput): PayloadManifest {
  ...
  return {
    ...
    artifacts: input.declaration.layout.map((entry) => ({
      ...
      render: entry.render,
      // Класс переносится из объявления как есть — упаковка его не толкует и не
      // подставляет умолчание: умолчание уже проставлено разбором формы.
      class: entry.class,
    })),
    files: input.files,
