/**
 * «ПРОВЕРЕННАЯ ДЕТАЛЬ, НОРМАЛИЗОВАННАЯ И С МАНИФЕСТОМ ВЫДАЧИ».
 *
 ...
 *
 * Обвес, не подходящий патрону, упаковываться не должен: иначе мы выдаём
 * заведомо не встающую деталь и узнаём об этом у потребителя (`kb:BASER2-9`).
 ...
 *
 * ## Отказ не оставляет за собой недоделанной детали
 *
 ...
 * как есть, и лишний файл в ней менял бы саму деталь ради разговора о ней.
 */
export function packPackage(source: string, options: PackOptions): PackReport {
  ...
  /** Убирает следы отказа: недоделанной детали на диске не остаётся. */
  const discard = (): void => {
  ...
  // ── Шаг 1. Деталь подходит патрону. Отказ приезжает от проверки как есть.
  const check = stage(
    'check',
    () => {
 ...
 * значило бы выдать деталь, про которую мы не знаем, что внутри.
 */
